...
def make_lstm_cell(mode, hiddenSize, index):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        # 트레이닝 모드에서 드랍아웃 추가
        cell = tf.contrib.rnn.DropoutWrapper(cell, output_keep_prob=DEFINES.dropout_rate)
    return cell
...
def Model(features, labels, mode, params):
    ...
    if TRAIN:
        embedding_encoder = tf.nn.dropout(embedding_encoder, DEFINES.dropout_rate)
    ...
    predict = tf.argmax(logits, 2)  # [Batch, Length, Word]
    ...
    ## Origin
    labels_ = tf.one_hot(labels, params['vocabulary_length'])  # 결과의 실제 레이블
